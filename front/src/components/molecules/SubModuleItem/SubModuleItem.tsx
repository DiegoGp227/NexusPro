...
export function SubModuleItem({ subModule }: SubModuleItemProps) {
  ...
  // Get translated name and description based on submodule id
  const subModuleName = t(`submodules.${subModule.id}` as any) || subModule.name;
  const subModuleDesc = t(`submodules.${subModule.id}.desc` as any) || subModule.description;
  ...
  if (isImplemented) {
    return (
      <Link
        href="/compras"
        className="group flex items-center gap-4 px-4 py-3 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-150"
      >
  ...
  return (
    <button
      onClick={() => alert(`${subModuleName} - In development`)}
      className="group flex items-center gap-4 px-4 py-3 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-150 w-full text-left"
    >
      <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors">
        <Icon name={subModule.icon} size="md" />
      </div>
