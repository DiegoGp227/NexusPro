  ...
  getAll: async (params?: { search?: string }): Promise<Proveedor[]> => {
    try {
      const url = params?.search
        ...
        : "/suppliers";
      console.log("[API] Fetching proveedores from:", url);
      const response = await api.get<Proveedor[]>(url);
      console.log("[API] Proveedores response:", response.data.length, "items");
      return response.data;
    } catch (error) {
      console.warn("[API] Backend no disponible, usando datos de prueba:", error);
