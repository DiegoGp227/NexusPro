...
export const proveedoresModel: ProveedoresModel = {
  ...
  fetchAll: thunk(async (actions, params) => {
    console.log("[Store] fetchAll proveedores called with params:", params);
    actions.setLoading(true);
    actions.setError(null);
    try {
      const data = await proveedoresService.getAll(params);
      console.log("[Store] fetchAll proveedores success:", data.length, "items");
      actions.setItems(data);
    } catch (error: unknown) {
      console.error("[Store] fetchAll proveedores error:", error);
